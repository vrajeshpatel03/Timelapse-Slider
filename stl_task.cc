...
stl_task::stl_task (const time_stamp& time_interval, base_text_serial* debug_port)
{
	...
	STL_DEBUG ("Creating task " << serial_number << endl);
...
bool stl_task::schedule (time_stamp& the_time)
{
	...
	switch (op_state)
	{
		...
		case (TASK_PENDING):
			...
			{										// has occurred
				STL_TRACE ("T" << serial_number << ":" << current_state << "-" 
					<< next_state << endl);
...
{
	STL_DEBUG ("Base run() method called for task " << serial_number << endl);
...
{
	STL_DEBUG ("ERROR in task " << serial_number << " state " << current_state
		<< ": " << message << endl << "Processing stopped." << endl);
